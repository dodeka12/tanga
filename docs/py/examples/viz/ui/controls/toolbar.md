# Four toolbars, one per alignment, stacked in a vertical split

**Keywords:** toolbar · alignment · split view · slider · dropdown · icon · separator · layout

Builds a vertical `~pytanga.viz.SplitView` with four equal panes.  Each
pane is a `~pytanga.viz.ToolbarView` holding the same mixed control set —
a labelled button, a `~pytanga.viz.SliderView`, a
`~pytanga.viz.DropdownView`, three icon-only buttons, and an
auto-oriented `~pytanga.viz.SeparatorView` — but with a different
...
```bash
uv run python py/examples/viz/ui/controls/toolbar.py
```

## Source

[`viz/ui/controls/toolbar.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/ui/controls/toolbar.py)
