...
`~pytanga.viz.ButtonView` updates them live via each view's `set_value`
...
Both are read-only content views that still carry a ``value``, so the leading
:class:`~pytanga.viz.ButtonView` updates them live via each view's ``set_value``
(the same ``control_update`` path every other control uses).
...
label_view = LabelView("label", value="Hello, world!", font_size=24)
