# the visualization-only Cylinder and Arc entities
...
- a solid `~pytanga.geometry.Cylinder` (both `align_center` modes),
- a partial `~pytanga.geometry.Arc` with a cone arrow tip at its end,
- a full-torus `~pytanga.geometry.Arc` (default `2π` radians).

## Run

```bash
uv run python py/examples/viz/entities/viz_entities.py
```

## Source

[`viz/entities/viz_entities.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/entities/viz_entities.py)
...
"""viz_entities.py — the visualization-only Cylinder and Arc entities.
