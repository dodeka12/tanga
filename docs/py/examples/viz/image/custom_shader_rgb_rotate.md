# Custom image shader that rotates RGB vectors

**Keywords:** image · ImageCanvas · custom shader · uniform · GLSL · RGB · rotation · drag

Replaces the `~pytanga.viz.ImageCanvas` standard shader with a custom
...
`~pytanga.viz.ImageCanvas.register_shader` and the uniform via
`~pytanga.viz.ImageCanvas.register_uniform`.

## Run

```bash
uv run python py/examples/viz/image/custom_shader_rgb_rotate.py
```

## Source

[`viz/image/custom_shader_rgb_rotate.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/image/custom_shader_rgb_rotate.py)
