# 2D camera, axes, and grid basics

**Keywords:** camera · 2D · Axes2D · Grid · View2DConfig

A minimal introduction to the three core 2D scene-building blocks:

* `~pytanga.viz.View2DConfig` — an orthographic camera view defined
  by a visible world rectangle (min/max bounds).
* `~pytanga.viz.Axes2D` — the two coordinate axes, controlled via a
  single convenience object.
* `~pytanga.viz.Grid` — a coordinate grid in the XY plane.

## Run

```bash
uv run python py/examples/viz/camera/axes_grid_2d.py
```

## Source

[`viz/camera/axes_grid_2d.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/camera/axes_grid_2d.py)
