# Stream a programmatic camera feed at 30 Hz over MJPEG

**Keywords:** camera · stream · mjpeg · 30fps · register_camera_stream

Publishes frames at 30 Hz through
`~pytanga.viz.Visualizer.register_camera_stream` and shows it in an
`~pytanga.viz.ImageCanvas` backed by `/stream/cam` (MJPEG over HTTP,
...
[`viz/camera/camera_stream.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/camera/camera_stream.py)
