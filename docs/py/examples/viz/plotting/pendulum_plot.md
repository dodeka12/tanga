# a swinging pendulum with a live angle-vs-time plot
...
Each frame the trail is appended and `~pytanga.viz.CoordinateSystem.update_plots`
re-syncs the path and the x axis, then `flush()` pushes the update.

## Run

```bash
uv run python py/examples/viz/plotting/pendulum_plot.py
```

## Source

[`viz/plotting/pendulum_plot.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/viz/plotting/pendulum_plot.py)
