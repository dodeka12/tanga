# Recover a rotor from point ↔ projection-ray matches
...
[`ga/tensor/rotor-point-on-ray_01.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/ga/tensor/rotor-point-on-ray_01.py)
...
try:
    ...
    print(
        "SciPy is not installed. Please run 'uv sync --extra example' or 'uv sync --group dev' to install."
    )
    exit(1)
...
def jacobean(R_a: np.ndarray) -> np.ndarray:
    ...
    dD_dR_t = (
        PP_t["kpq"] * dPlane_dR_t["pin_"] * Plane_t["qn_"]
        + PP_t["kpq"] * Plane_t["pn_"] * dPlane_dR_t["qin_"]
    )["->kin"]
    jac_t = dD_dR_t.sum("n")
    return jac_t.data[0]
