...
- `Expression.inv` — inverts the square, invertible map, returning a new
  expression that maps `B` back to `X`.
- `Expression.lstsq` — solves `M · vec(X) = vec(B)` numerically with an
...
[`ga/expression/solve_ax_b.py`](https://github.com/dodeka12/tanga/blob/main/py/examples/ga/expression/solve_ax_b.py)
