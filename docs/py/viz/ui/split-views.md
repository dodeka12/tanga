...
`LabelView`, `MarkdownView`, and `LogView` are three read-only content views that
render text, rendered markdown (with KaTeX math), and a live log inside a layout
— no scene required.  They are documented on their own page:
[Display Views](display-views.md).
...
- [Multi-Scene](../visualizer/multi-scene.md) — named scenes and their per-scene URLs
- [Visualizer API](../visualizer/visualizer.md) — the full `Visualizer` method reference
- [Animation](../visualizer/animation.md) — frame streaming and keyframe timelines
