...
typedef Tan::CCongruence_Float<TValue> TCongruence;

void Test_Matrix_GE_01()
{
	try
	{
		TCongruence xIdent;
		TMatrix matA(2, 2, { 1, 2, 1, 2 });
		TMatrix matAi;

		cout << "A: " << ToString(matA) << endl;

		EMatrixResult eRes = TAlgoGE::Inverse(matAi, matA, xIdent);
...
int main(int, char**) { Test_Matrix_GE_01(); return 0; }
