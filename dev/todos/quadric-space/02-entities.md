...
- [x] **2.2 — `hyperbola.py`** — `Hyperbola(center, dir1, dir2, a, b)` (transverse /
  conjugate semi-axes; `dir1`/`dir2` are `Direction`).

- [x] **2.3 — `parabola.py`** — `Parabola(vertex, direction, p)` (focal parameter).

- [x] **2.4 — `line_pair.py`** — `LinePair(line1, line2)` (intersecting) and
  `ParallelLinePair(line1, line2)` (parallel); coerce via existing `Line`.

- [x] **2.5 — `point_set.py`** — `PointSet(points, kind=None)` holding
  `tuple[Point, ...]`; optional `kind` (`single/pair/triplet/quadruplet/n_tuple`).

- [x] **2.6 — exports** — add the new classes to `entities/__init__.py` and
...
- [x] **2.7 — Tests** — constructor defaults/coercion, `__repr__`, cached
  `Conic`/`Quadric3D` derived properties for a known circle/ellipsoid matrix,
  `LinePair` vs `ParallelLinePair` classification fields.

- [x] **2.8 — Validate** — `uv run pytest py/tests/geometry/test_conic_entities.py -q`.
