...
- [x] In the `_jupyter` branch of `display_static`, escape the HTML
      (`html.escape(html, quote=True)`) and return
      `HTML(f'<iframe srcdoc="{escaped}" width="{width}" height="{height}" …></iframe>')`.
- [x] Honor the currently-ignored `width`/`height` parameters (normalize
      `int`/`str` to a CSS value + `px`).
- [x] Keep the non-Jupyter branch as-is (temp file + `webbrowser.open`), which
      becomes the basis for `open_snapshot()` in Phase 4.

## Unit tests

- [x] `py/tests/viz/test_display.py` (new):
  - [x] With `viz._jupyter = True`, `display_static()` returns an
        `IPython.display.HTML` whose `.data` starts with `<iframe srcdoc=`.
  - [x] The `srcdoc` content is HTML-escaped (`&lt;!DOCTYPE html&gt;`) and does
        not contain a bare `<style>`/`<html>`/`<body>` tag.
  - [x] With `viz._jupyter = False`, it writes a temp file and returns `None`
        (patch `webbrowser.open`).

## Verification

- [x] `uv run pytest py/tests/viz/test_display.py` passes.
- [x] Manual: in a Jupyter notebook, `viz.display_static()` renders inline and
