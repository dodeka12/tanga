...
- [x] Factor the "resolved leaf" out of `VizSceneObject.serialize()` into a
...
- [x] Add `content` to `VizSceneObject.patch()`:
      `{ "id", "aspect": "content", "value": self._serialize_content() }`.
- [x] `set_entity(entity)`: mark `content` when
      `type(entity).__name__ == self.kind`, otherwise `full`.

### Python — `py/pytanga/viz/scene.py`

- [x] `flush()`: iterate aspects `("full", "style", "transform", "content")`.

### Frontend — `py/pytanga/viz/templates/viewer.js`

- [x] Delete `entityMeshes`, `entityData`, `labelObjects`; redefine
      `sceneObjects` entries as `{ obj, mesh, data, layer, el? }`.
- [x] `upsertObject`: store `mesh` + `data` in the entry (both layers).
- [x] `applyObjectPatch`:
  - [x] `content` → new `updateEntityContent(id, value)`.
  - [x] `style` → merge into `entry.data` (replaces `entityData`).
- [x] New `updateEntityContent(id, content)`:
  - [x] lookup `sceneObjects.get(id)` → `{ obj, mesh, data }`; if absent, return.
  - [x] `updateEntityMesh(mesh, content, prev = data)`:
    - [x] `true` → `entry.data = { ...data, ...content }` (in-place done).
    - [x] `false` → `createEntityMesh({ ...data, ...content })` and swap the
          new mesh in, keeping `obj`/parent/transform:
      - [x] identity transform (`entry.obj === entry.mesh`): remove the old
            node, add the new mesh under the same parent, set
            `entry.obj = entry.mesh = newMesh`.
      - [x] wrapped (`entry.obj` is a `Group`): `removeEntityMesh(entry.mesh)`,
            `entry.obj.add(newMesh)`, `entry.mesh = newMesh`.
      - [x] update `entry.data = { ...data, ...content }`, and
            re-`registerInteractive(id, entry.obj, data.interaction)` when
            `data.interaction` exists.
- [x] `removeSceneObject(id)`: single `sceneObjects` lookup, branch on `layer`
      ...
      `cancelTween`.
- [x] `clear_all`: clear only `sceneObjects` (plus existing reset steps).
- [x] `handleAnimate` / render loop: pass `sceneObjects` to the animator.

### Legacy removal — `viewer.js`

- [x] Remove `updateEntity()`, `inPlaceUpdate()`, `upsertLabel()`.
- [x] Remove the `msg.entities` and `msg.labels` branches in `scene_update`.
- [x] `scene_update` objects loop →
      `for (const obj of msg.objects) upsertObject(obj);` (drop the
      `hasSceneGraph` routing).
- [x] Remove all `.set/.delete/.get/.clear` on the three deleted maps.

### Frontend — `py/pytanga/viz/templates/animator.js`

- [x] `startTween(id, target, duration, easing, map)` and
      `updateTweens(map)`: read `map.get(id)?.obj` (map is now `sceneObjects`).

### Frontend — `py/pytanga/viz/templates/view_mode.js`

- [x] `fitCamera(sceneObjects, ...)`: iterate entries and
      `box.expandByObject(entry.obj)` only for `entry.layer === 'scene' &&
      entry.obj`.

### Docs / notes

- [x] Update `docs/py/viz/scene-graph.md`: `ref.entity = ...` → `content`.
- [x] Append an entry to `IMPLEMENTATION-NOTES.md`.
- [x] Update `README.md` phases table with Phase 13.
...
- [x] `py/tests/viz/test_nodes.py`: `test_set_entity_marks_content` (same kind)
      and `full` on kind change.
- [x] `py/tests/viz/test_object_ref.py`: `test_entity_get_set` asserts
      `content` (was `full`).
- [x] `py/tests/viz/test_node_serialization.py`: `test_aspect_content_patch`
      (shape `{id, aspect:"content", value:{kind, ...geometry}}`, no
      `parent_id/transform/visible`).
- [x] `py/tests/viz/test_frontend_transform_shape.py`: `test_content_patch_shape`.
- [x] `node --check` on `viewer.js`, `animator.js`, `view_mode.js`.
- [x] Manual smoke: `demo_scene_graph.py`, `demo_animation_orbit.py` — verify
      parented + non-identity-transform objects, `attach_to` labels, tweens,
      camera fit, and reconnect.

## Verification

- [x] `uv run pytest py/tests/viz -q` passes.
- [x] `viz.update_entity(...)` on a same-kind entity mutates in place (no
      flicker/reparent); a kind change rebuilds via `full`.
- [x] No `entityMeshes`/`entityData`/`labelObjects`/`updateEntity`/
      `inPlaceUpdate`/`upsertLabel` references remain in `viewer.js`.
- [x] Static/animated export still works (untouched).
