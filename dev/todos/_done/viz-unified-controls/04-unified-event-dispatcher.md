...
- [x] **4.1 — Backend `_dispatch_event(target, event, scene, data)`**
...
- [x] **4.2 — Fold banner/editor handlers into the registry**
...
- [ ] **4.3 — Frontend sends the unified envelope**
  > **Deferred.** The dispatcher is unified backend-side; the frontend still
  > sends legacy `control:*` / `banner_closed` / `editor_closed` /
  > `file_browser_*` messages, which route through `_dispatch_event`. A wire
  > migration is only worth doing once the remaining phases need it.
