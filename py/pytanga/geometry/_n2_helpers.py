...
"""Shared N2 helper functions for creation and analysis modules.

Provides algebraic extraction of null-vector components (eini and eo)
...
def eucl_part(mv: MV, einf: MV, eo: MV) -> tuple[float, float]:
    """Euclidean (e₁, e₂) coefficients of a grade-1 blade.
...
def translator_coeffs(mv: MV, basis: Algebra) -> tuple[float, float]:
    """Extract (dx, dy) from a translator versor via algebraic extraction.
...
def has_translator_components(mv: MV, basis: Algebra) -> bool:
    ...
    try:
        tx, ty = translator_coeffs(mv, basis)
    except ValueError:
        return False
    return abs(tx) + abs(ty) > 1e-15
