...
2D: ``_embed_point(basis, x, y)``
    ``x b₁ + y b₂ + (√2/2) b₃ + (√2/2)x² b₄ + (√2/2)y² b₅ + x y b₆``

3D: ``_embed_point(basis, x, y, z)``
...
def _embed_point(
    ...
    ``_embed_point`` is the Euclidean-rescaled ``D_op(x) = x xᵀ``: its inner
    ...
    if dim == 6:
        if y is None or z is not None:
            raise ValueError(
                "2D _embed_point(basis, x, y) takes exactly two coordinates"
            )
    ...
    if dim == 10:
        if y is None or z is None:
            raise ValueError(
                "3D _embed_point(basis, x, y, z) takes exactly three coordinates"
            )
        return basis.multivector(
            {
                1: x,
