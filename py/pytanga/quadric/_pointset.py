...
Quadric-space point recovery from rank-1 embeddings and simple blades, plus the
thesis pencil method for intersecting two conics.  ``Point`` / ``PointSet`` are
imported lazily, mirroring :mod:`pytanga.quadric.refine`.
"""
...
def point_from_embedding(mv, dim: int) -> Point:
    ...
    if dim == 6:
        return E.Point(float(matrix[0, 2] / w), float(matrix[1, 2] / w), 0.0)
    return E.Point(
        float(matrix[0, 3] / w),
        float(matrix[1, 3] / w),
        float(matrix[2, 3] / w),
...
def pointset_from_blade(mv) -> PointSet:
    ...
    return E.PointSet(points, kind=_kind_for(len(points)))
...
def _point_from_rank1_matrix(m: np.ndarray, dim: int) -> Point | None:
    ...
    if abs(w) < 1e-12:
        return None
    if dim == 6:
        return E.Point(float(v[0] / w), float(v[1] / w), 0.0)
...
def two_conic_intersection(A, B) -> PointSet:
    ...
    if abs(float(np.linalg.det(B))) < 1e-12:
        if abs(float(np.linalg.det(A))) < 1e-12:
            return E.PointSet([], kind="n_tuple")
