...
"""demo_sdf_booleans.py — per-object CSG combine modes.

Demonstrates the ``combine=``/``polarity=`` API across *separate* scene objects
(distinct from :class:`Composed`, which ``demo_sdf_composed.py`` covers): a
...
Run with:  uv run python py/examples/viz/demo_sdf_booleans.py
"""
