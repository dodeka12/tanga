...
class TestRightContraction:
    def test_rc_vector_vector(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 2.0})
        e2 = alg.multivector({"e2": 3.0})
        result = e1.rc(e2)
        assert result["s"] == pytest.approx(0.0)

    def test_rc_bivector_vector(self, alg):  # noqa: ANN001, ANN201
    ...
    def test_rc_vector_bivector_vanishes(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
        e12 = alg.multivector({"e12": 1.0})
        result = e1.rc(e12)
        assert result.is_zero

    def test_rc_relation_to_ip(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
...
# ═══════════════════════════════════════════════════════════════════════════
class TestGpMin:
    def test_gp_min_vectors(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 2.0})
        e2 = alg.multivector({"e2": 3.0})
        result = e1.gp_min(e2)
        assert result["s"] == pytest.approx(0.0)

    def test_gp_min_same_vector(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 2.0})
        result = e1.gp_min(e1)
        assert result["s"] == pytest.approx(4.0)

    def test_gp_min_bivector_vector(self, alg):  # noqa: ANN001, ANN201
        e12 = alg.multivector({"e12": 1.0})
        e1 = alg.multivector({"e1": 2.0})
        result = e12.gp_min(e1)
        assert result["e2"] == pytest.approx(-2.0)

    def test_gp_min_raises_on_multigrade(self, alg):  # noqa: ANN001, ANN201
...
# ═══════════════════════════════════════════════════════════════════════════
class TestGpMax:
    def test_gp_max_vectors_equals_op(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 2.0})
        e2 = alg.multivector({"e2": 3.0})
        result = e1.gp_max(e2)
    ...
    def test_gp_max_bivector_vector(self, alg):  # noqa: ANN001, ANN201
        e12 = alg.multivector({"e12": 1.0})
        e3 = alg.multivector({"e3": 2.0})
        result = e12.gp_max(e3)
        assert result["e123"] == pytest.approx(2.0)

    def test_gp_max_vector_bivector(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 2.0})
        e23 = alg.multivector({"e23": 3.0})
        result = e1.gp_max(e23)
        assert result["e123"] == pytest.approx(6.0)

    def test_gp_max_raises_on_multigrade(self, alg):  # noqa: ANN001, ANN201
