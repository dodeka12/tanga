...
class TestConstants:
    def test_algebra_dim(self, alg):  # noqa: ANN001, ANN201
        assert alg.algebra_dim == 8  # 2^3

    def test_pseudoscalar_id(self, alg):  # noqa: ANN001, ANN201
...
class TestBasisVectorSquares:
    ...
    def test_vector_squares_to_positive_scalar(self, alg, name):  # noqa: ANN001, ANN201
        e = alg.multivector({name: 1.0})
        sq = e * e
        assert abs(sq["s"] - 1.0) < 1e-9
        assert sq["e1"] == pytest.approx(0.0, abs=1e-9)

    def test_e1_e2_anticommute(self, alg):  # noqa: ANN001, ANN201
...
class TestOuterProduct:
    def test_e1_wedge_e2_gives_e12(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
        e2 = alg.multivector({"e2": 1.0})
        result = e1 ^ e2
        assert result["e12"] == pytest.approx(1.0, abs=1e-9)

    def test_wedge_with_self_is_zero(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
        result = e1 ^ e1
...
class TestInverse:
    def test_inv_e12(self, alg):  # noqa: ANN001, ANN201
        """In G(3,0), e12 * e12 = -1, so inv(e12) = -e12."""
        e12 = alg.multivector({"e12": 1.0})
        inv_e12 = ~e12
        assert inv_e12["e12"] == pytest.approx(-1.0, abs=1e-9)

    def test_mv_times_inv_is_scalar_one(self, alg):  # noqa: ANN001, ANN201
...
class TestOperatorOverloads:
    def test_mul_operator(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
        e2 = alg.multivector({"e2": 1.0})
        assert (e1 * e2)["e12"] == pytest.approx(1.0, abs=1e-9)

    def test_xor_operator(self, alg):  # noqa: ANN001, ANN201
        e1 = alg.multivector({"e1": 1.0})
        e2 = alg.multivector({"e2": 1.0})
        assert (e1 ^ e2)["e12"] == pytest.approx(1.0, abs=1e-9)
...
class TestRepr:
    def test_repr_nonzero(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0})
        assert "e1" in repr(mv)

    def test_repr_zero(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector()
        assert repr(mv) == "0"
...
class TestMultivectorTupleKeys:
    ...
    def test_invalid_index_zero_raises(self, alg):  # noqa: ANN001, ANN201
        # 0 is only valid as a singleton (0,) meaning scalar
        with pytest.raises(ValueError, match="scalar"):
            alg.multivector({(0, 1): 1.0})

    def test_index_out_of_range_raises(self, alg):  # noqa: ANN001, ANN201
...
class TestPrecision:
    ...
    def test_default_precision(self, alg):  # noqa: ANN001, ANN201
        assert alg.precision == 1e-10

    def test_custom_precision(self):  # noqa: ANN201
        a = pytanga.Algebra(dim=3, sig=0, precision=1e-8)
        assert a.precision == 1e-8

    def test_set_precision(self, alg):  # noqa: ANN001, ANN201
        alg.precision = 1e-12
        assert alg.precision == 1e-12
        alg.precision = 1e-10  # restore default for other tests

    def test_is_zero_with_tiny_values(self, alg):  # noqa: ANN001, ANN201
    ...
    def test_is_zero_above_precision(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1e-9})
        assert mv.is_zero is False

    def test_is_scalar_with_tiny_non_scalar(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0, "e1": 1e-13})
        # Default precision is 1e-10; e1 is tiny, so it is scalar
        assert mv.is_scalar is True

    def test_is_scalar_with_non_scalar_above_precision(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0, "e1": 1e-9})
        assert mv.is_scalar is False

    def test_prune_removes_tiny_coefficients(self, alg):  # noqa: ANN001, ANN201
    ...
    def test_prune_keeps_above_precision(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0, "e1": 1e-9})
        mv.prune()
        assert mv["s"] == pytest.approx(5.0)
        assert mv["e1"] == pytest.approx(1e-9)

    def test_custom_precision_prune(self):  # noqa: ANN201
...
class TestMultivectorStringInput:
    ...
    def test_scalar_only(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("7")
        assert mv["s"] == pytest.approx(7.0)

    def test_single_blade(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("e1")
        assert mv["e1"] == pytest.approx(1.0)

    def test_blade_with_coefficient(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("4 e2")
        assert mv["e2"] == pytest.approx(4.0)

    def test_negative_coefficient(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("-3 e1")
        assert mv["e1"] == pytest.approx(-3.0)

    def test_bare_negative_blade(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("-e2")
        assert mv["e2"] == pytest.approx(-1.0)

    def test_sum_of_terms(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("2.3 + 4 e2 + 5 e1,2")
        assert mv["s"] == pytest.approx(2.3)
        assert mv["e2"] == pytest.approx(4.0)
        assert mv["e12"] == pytest.approx(5.0)

    def test_comma_separated_high_dim_blade(self, alg):  # noqa: ANN001, ANN201
        # e1,2 = e12 in G(3)
        mv = alg.multivector("e1,2")
        assert mv["e12"] == pytest.approx(1.0)

    def test_negative_blade_no_coeff(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("e1 - e2")
        assert mv["e1"] == pytest.approx(1.0)
        assert mv["e2"] == pytest.approx(-1.0)

    def test_star_separator(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("3*e1")
        assert mv["e1"] == pytest.approx(3.0)

    def test_scalar_plus_blade(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector("1 + e12")
        assert mv["s"] == pytest.approx(1.0)
        assert mv["e12"] == pytest.approx(1.0)

    def test_compact_bivector_name(self, alg):  # noqa: ANN001, ANN201
        # "e12" without comma — compact form valid for dim ≤ 9
        mv = alg.multivector("5 e12")
        assert mv["e12"] == pytest.approx(5.0)

    def test_full_multivector(self, alg):  # noqa: ANN001, ANN201
...
class TestReversedBladeNames:
    ...
    def test_string_reversed_bivector(self, alg):  # noqa: ANN001, ANN201
        assert (alg("1 e31") + alg("1 e13")).is_zero
        assert alg("1 e31")["e13"] == pytest.approx(-1.0)

    def test_string_reversed_trivector(self, alg):  # noqa: ANN001, ANN201
        mv = alg("1 e321")
        assert mv["e123"] == pytest.approx(-1.0)

    def test_bracket_read_sign(self, alg):  # noqa: ANN001, ANN201
        mv = alg("2 e13")
        assert mv["e31"] == pytest.approx(-2.0)
        assert mv["e13"] == pytest.approx(2.0)

    def test_bracket_write_sign(self, alg):  # noqa: ANN001, ANN201
        mv = alg()
        mv["e31"] = 3.0
        assert mv["e13"] == pytest.approx(-3.0)
        assert mv["e31"] == pytest.approx(3.0)

    def test_tuple_key_reversed(self, alg):  # noqa: ANN001, ANN201
        a = alg({(3, 1): 2.0})
        b = alg({(1, 3): 2.0})
        assert (a + b).is_zero
