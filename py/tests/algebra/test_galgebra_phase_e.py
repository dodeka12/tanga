...
class TestIsVector:
    def test_is_vector_true(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0, "e2": 2.0})
        assert mv.is_vector is True

    def test_is_vector_false_mixed(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0, "e12": 2.0})
        assert mv.is_vector is False

    def test_is_vector_scalar_not_vector(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0})
        assert mv.is_vector is False


class TestIsBase:
    def test_is_base_true(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0})
        assert mv.is_base is True

    def test_is_base_scalar(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 1.0})
        assert mv.is_base is True

    def test_is_base_false_coefficient(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 2.0})
        assert mv.is_base is False

    def test_is_base_false_multiple_blades(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0, "e2": 1.0})
        assert mv.is_base is False


class TestIsBlade:
    def test_is_blade_scalar(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0})
        assert mv.is_blade is True

    def test_is_blade_vector(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 3.0, "e2": 4.0})
        assert mv.is_blade is True

    def test_is_blade_bivector(self, alg):  # noqa: ANN001, ANN201
        # e12 is a blade (simple bivector)
        mv = alg.multivector({"e12": 1.0})
        assert mv.is_blade is True

    def test_is_blade_false_mixed_grade(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0, "e12": 2.0})
        assert mv.is_blade is False


class TestComponents:
    def test_components(self, alg):  # noqa: ANN001, ANN201
    ...
    def test_components_zero(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector()
        assert mv.components() == []


class TestGetCoefs:
    def test_get_coefs_grade1(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0, "e2": 2.0, "e3": 3.0})
        coefs = mv.get_coefs(1)
        assert coefs == pytest.approx([1.0, 2.0, 3.0])

    def test_get_coefs_grade0(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 5.0})
        coefs = mv.get_coefs(0)
        assert coefs == pytest.approx([5.0])

    def test_get_coefs_missing_grade(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 1.0})
