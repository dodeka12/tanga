...
class TestGradeConj:
    ...
    def test_vector(self, alg):  # noqa: ANN001, ANN201
        # For grade 1: (−1)^(1·2/2) = (−1)^1 = −1, and rev(k=1) = +1 → net −1
        mv = alg.multivector({"e1": 3.0})
        result = mv.grade_conj()
        assert result["e1"] == pytest.approx(-3.0)

    def test_bivector(self, alg):  # noqa: ANN001, ANN201
        # grade_conj = ginvol.rev. For k=2: ginvol(−)=+, rev(−)=− → net −1
        mv = alg.multivector({"e12": 4.0})
        result = mv.grade_conj()
        assert result["e12"] == pytest.approx(-4.0)

    def test_trivector(self, alg):  # noqa: ANN001, ANN201
        # k=3: ginvol(−)=−, rev(−)=− → net +1
        mv = alg.multivector({"e123": 2.0})
        result = mv.grade_conj()
        assert result["e123"] == pytest.approx(2.0)

    def test_grade_conj_equals_ginvol_rev(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 1.0, "e1": 2.0, "e12": 3.0, "e123": 4.0})
        assert (mv.grade_conj() - mv.grade_involution().rev()).is_zero

    def test_grade_conj_distinct_from_conj(self, alg):  # noqa: ANN001, ANN201
...
# ═══════════════════════════════════════════════════════════════════════════
class TestQForm:
    def test_euclidean_qform_equals_mag2(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e1": 3.0, "e2": 4.0})
        assert mv.qform() == pytest.approx(mv.mag2)
        assert mv.qform() == pytest.approx(25.0)

    def test_qform_scalar(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 7.0})
        assert mv.qform() == pytest.approx(49.0)

    def test_qform_bivector(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"e12": 2.0})
        assert mv.qform() == pytest.approx(4.0)

    def test_qform_equals_rev_sp(self, alg):  # noqa: ANN001, ANN201
...
# ═══════════════════════════════════════════════════════════════════════════
class TestEvenOdd:
    def test_even_extracts_scalar_and_bivector(self, alg):  # noqa: ANN001, ANN201
    ...
    def test_odd_extracts_vector_and_trivector(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 1.0, "e1": 2.0, "e2": 3.0, "e12": 4.0, "e123": 5.0})
        o = mv.odd()
    ...
    def test_even_plus_odd_equals_original(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 1.0, "e1": 2.0, "e12": 3.0, "e123": 4.0})
        assert (mv.even() + mv.odd() - mv).is_zero

    def test_even_equals_grade_proj_sum(self, alg):  # noqa: ANN001, ANN201
        mv = alg.multivector({"s": 1.0, "e1": 2.0, "e2": 3.0})
        expected = mv.grade(0) + mv.grade(2)
        assert (mv.even() - expected).is_zero

    def test_odd_equals_grade_proj_sum(self, alg):  # noqa: ANN001, ANN201
