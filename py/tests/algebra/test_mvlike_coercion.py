...
class TestMVLikeCoercion:
    def test_solve_scalar_coercion(self, alg_float):  # noqa: ANN001, ANN201
        a = alg_float({"e1": 1.0, "e2": -2.0, 0: 0.5})
        x1 = solve(a, 1.0)
        x2 = solve(a, alg_float({0: 1.0}))
        assert x1.to_dict() == x2.to_dict()

    def test_solve_string_coercion(self, alg_float):  # noqa: ANN001, ANN201
        a = alg_float({"e1": 1.0, "e2": -2.0, 0: 0.5})
        x1 = solve(a, "1")
        x2 = solve(a, 1.0)
        assert x1.to_dict() == x2.to_dict()

    def test_to_matrix_scalar(self, alg_float):  # noqa: ANN001, ANN201
    ...
    def test_solve_mod_scalar_coercion(self, alg_int):  # noqa: ANN001, ANN201
