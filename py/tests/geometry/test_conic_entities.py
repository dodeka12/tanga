...
def _ellipsoid_matrix():  # noqa: ANN202
    # x^2/4 + y^2/9 + z^2/16 = 1
    return np.diag([0.25, 1.0 / 9.0, 1.0 / 16.0, -1.0])


def _sphere_matrix():  # noqa: ANN202
    # (x - 1)^2 + (y - 2)^2 + (z - 3)^2 = 4
    return np.array(
        [
            [1.0, 0.0, 0.0, -1.0],
...
def _assert_point(p, x, y, z):  # noqa: ANN001, ANN202
...
class TestConic:
    ...
    def test_quadric2d_alias(self):  # noqa: ANN201
...
class TestConicEntities:
    ...
    def test_parallel_line_pair_is_distinct(self):  # noqa: ANN201
        l1 = Line(Point(0.0, 0.0, 0.0), Direction(1.0, 0.0, 0.0))
    ...
    def test_point_set(self):  # noqa: ANN201
