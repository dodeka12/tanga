...
def test_cylinder_defaults():  # noqa: ANN201
    c = Cylinder()
...
def test_cylinder_field_population_and_coercion():  # noqa: ANN201
...
def test_cylinder_align_center_coercion():  # noqa: ANN201
...
def test_arc_defaults():  # noqa: ANN201
    a = Arc()
...
def test_arc_start_direction_auto_computed_orthogonal_and_unit():  # noqa: ANN201
...
def test_arc_arrow_explicit_values():  # noqa: ANN201
...
def test_arc_arrow_defaults_kept_when_show_arrow_true():  # noqa: ANN201
...
def test_cylinder_not_convertible_to_mv():  # noqa: ANN201
    alg = BasisN3()
    with pytest.raises(TypeError, match="Expected Entity or Operator"):
        create(alg, Cylinder())


def test_disk_defaults():  # noqa: ANN201
    d = Disk()
...
def test_disk_field_population_and_coercion():  # noqa: ANN201
...
def test_partial_disk_defaults():  # noqa: ANN201
    p = PartialDisk()
...
def test_partial_disk_start_direction_auto_computed_orthogonal_and_unit():  # noqa: ANN201
...
def test_box_field_population_and_coercion():  # noqa: ANN201
    b = Box(center=Point(1, 2, 3), size=(2, 3, 4))
    assert b.center == Point(1, 2, 3)
    assert b.size == (2.0, 3.0, 4.0)


def test_ellipsoid_defaults():  # noqa: ANN201
    e = Ellipsoid()
...
def test_regular_polygon_defaults():  # noqa: ANN201
    p = RegularPolygon()
    assert p.center == Point(0, 0, 0)
    assert p.radius == pytest.approx(1.0)
    assert p.sides == 6
...
def test_regular_polygon_sides_validated():  # noqa: ANN201
    with pytest.raises(ValueError):
        RegularPolygon(sides=2)


def test_regular_polygon_sides_coerced_to_int():  # noqa: ANN201
    p = RegularPolygon(sides=6.0)
    assert p.sides == 6
    assert isinstance(p.sides, int)


def test_regular_polygon_factory():  # noqa: ANN201
    h = regular_polygon(6)
...
def test_new_viz_entities_not_convertible_to_mv(entity):  # noqa: ANN001, ANN201
    alg = BasisN3()
    with pytest.raises(TypeError, match="Expected Entity or Operator"):
        create(alg, entity)


def test_arc_not_convertible_to_mv():  # noqa: ANN201
    alg = BasisN3()
    with pytest.raises(TypeError, match="Expected Entity or Operator"):
        create(alg, Arc())
