...
class TestBladeMask:
    def test_int_ids_sorted_deduped(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, [4, 1, 1, 2])
        assert m.ids == [1, 2, 4]

    def test_index_lookup(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, [4, 1, 1, 2])
        assert m.index(2) == 1

    def test_contains(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, [4, 1, 1, 2])
        assert 4 in m
        assert 7 not in m
    ...
    def test_from_array(self, alg_float):  # noqa: ANN001, ANN201
        e1 = alg_float("e1")
        e2 = alg_float("e2")
        e12 = alg_float("e12")
    ...
    def test_from_str_expression(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, "1 + 2 e3 - e13")
        assert m.ids == [0, 4, 5]

    def test_from_str_simple(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, "e1 - e2")
        assert m.ids == [1, 2]

    def test_from_list_of_strings(self, alg_float):  # noqa: ANN001, ANN201
        m = BladeMask(alg_float, ["e12", "1 + e13"])
        assert m.ids == [0, 3, 5]

    def test_grades_scalar(self, alg_float):  # noqa: ANN001, ANN201
        assert BladeMask(alg_float, grades=[0]).ids == [0]

    def test_grades_bivectors(self, alg_float):  # noqa: ANN001, ANN201
        assert BladeMask(alg_float, grades=[2]).ids == [3, 5, 6]

    def test_grades_even_subalgebra(self, alg_float):  # noqa: ANN001, ANN201
        assert BladeMask(alg_float, grades=[0, 2]).ids == [0, 3, 5, 6]

    def test_combined_ids_and_grades(self, alg_float):  # noqa: ANN001, ANN201
    ...
    def test_intersection(self, alg_float):  # noqa: ANN001, ANN201
        a = BladeMask(alg_float, [0, 1, 2])
        b = BladeMask(alg_float, [0, 3])
        assert a.intersection(b).ids == [0]

    def test_union_cross_algebra_raises(self, alg_float, alg_int):  # noqa: ANN001, ANN201
        a = BladeMask(alg_float, [1])
        b = BladeMask(alg_int, [1])
        with pytest.raises(AssertionError):
            a.union(b)

    def test_product_blade_mask_gp(self, alg_float, mask_A_float):  # noqa: ANN001, ANN201
        out = product_blade_mask(mask_A_float, mask_A_float, complete=True)
        assert isinstance(out, BladeMask)
        assert len(out) >= 1

    def test_product_blade_mask_cross_algebra_raises(self, alg_float, alg_int):  # noqa: ANN001, ANN201
        float_mask = BladeMask(alg_float, [1])
        wrong_mask = BladeMask(alg_int, [1])
        with pytest.raises(AssertionError):
            product_blade_mask(float_mask, wrong_mask)

    def test_unknown_product_raises(self, alg_float, mask_A_float):  # noqa: ANN001, ANN201
        with pytest.raises(ValueError):
            product_blade_mask(mask_A_float, mask_A_float, product="xy")

    def test_inverse_blade_mask_ip_both_directions(self, alg_float):  # noqa: ANN001, ANN201
    ...
    def test_inverse_blade_mask_ip_independent_of_left(self, alg_float):  # noqa: ANN001, ANN201
        a = BladeMask(alg_float, "e12")
        c = BladeMask(alg_float, "e1 + e2 + e3")
    ...
    def test_inverse_blade_mask_op_independent_of_left(self, alg_float):  # noqa: ANN001, ANN201
